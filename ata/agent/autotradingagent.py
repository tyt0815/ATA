                ...
                for target in buying_candidates:
                    ...
                    if self.exchange.balance['KRW']['total'] > 0 and self._is_buy_timing(ohlcv):
                        ...
                        if not target in buy_cnt:
                            buy_cnt[target] = 0
                        buy_cnt[target] += 1    
                        krw = min(self.exchange.get_total_balance() / 5 * buy_cnt[target], self.exchange.balance['KRW']['free'] - 100)
                        if krw > 6000:
                            ...
                            log(f'Buy  {target} at {buy_order["price"]}({self.exchange.get_total_balance()}) Debug - buy_cnt["{target}"] = {buy_cnt[target]}')
